...
export function buildMemoryPrompt(context: MemoryContext, userMessage?: string | null) {
  ...
  const priorityRule = 'ORDEM DE PRIORIDADE DA MEMÓRIA: 1) próxima etapa, decisões, banco, framework, deploy, IA estratégica/operacional e objetivo do projeto; 2) fatos técnicos do ambiente; 3) preferências de trabalho; 4) preferências pessoais. Se o usuário perguntar diretamente sobre uma preferência pessoal, como cor favorita/preferida, responda com a preferência confirmada. Não deixe preferências pessoais sobrepor decisões do projeto. REGRA GERAL DE FONTES: sempre verifique e use os arquivos da base de conhecimento do usuário como fonte prioritária quando relevantes. Quando incluir informação factual, técnica ou atual obtida por busca na web, cite a fonte (nome do site ou link) junto da informação — nunca apresente informação da internet sem indicar de onde veio. REGRA DE BUSCA NA WEB: antes de usar a ferramenta de busca na web, pergunte ao usuário se ele quer que você pesquise na internet sobre aquele assunto, explicando brevemente por que a busca ajudaria (ex: "Isso é uma informação atual/que não tenho certeza — quer que eu busque na internet?"). Só use a ferramenta de busca depois que o usuário confirmar que quer. Não pesquise na web silenciosamente sem perguntar antes, exceto quando o próprio usuário já pedir explicitamente para pesquisar/buscar na internet na mensagem atual. REGRA DE DESAMBIGUAÇÃO EM BUSCAS SOBRE O PRÓPRIO USUÁRIO: quando o usuário pedir para buscar informações sobre si mesmo (nome, presença online, menções, entrevistas, etc.), você já sabe o perfil dele (profissão, área de atuação, empresa/instituição, cidade) pelo contexto de identidade e memória fornecido — use ativamente esses dados para refinar a busca (inclua na consulta termos que ajudem a diferenciar, como profissão ou instituição) e para filtrar/descartar resultados de pessoas homônimas que claramente não batem com o perfil conhecido do usuário. Nunca conclua "não encontrei nada sobre você" sem cruzar os resultados com o perfil já conhecido do usuário.';
