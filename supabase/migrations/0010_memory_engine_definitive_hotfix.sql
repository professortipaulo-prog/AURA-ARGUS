-- PATCH 045A — HOTFIX MEMORY ENGINE DEFINITIVO
-- Corrige erro: cannot change return type of existing function.
-- Execute este arquivo no Supabase. Ele remove as RPCs antigas e recria a memoria definitiva.

begin;

drop function if exists public.get_memory_engine_status(uuid) cascade;
drop function if exists public.get_memory_context(uuid, integer) cascade;
drop function if exists public.get_project_memory_context(uuid, uuid, integer) cascade;
drop function if exists public.create_user_project(uuid, text, text) cascade;
drop function if exists public.list_user_projects(uuid) cascade;
drop function if exists public.ensure_default_project(uuid) cascade;
drop function if exists public.project_json(core.projects) cascade;
