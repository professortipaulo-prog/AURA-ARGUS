...
import { generateWordSearch } from '@/lib/study/generation';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const game = await generateWordSearch(subject, persona);
    if (!game) {
      return NextResponse.json({ ok: false, error: 'Não consegui montar o caça-palavras agora. Tente de novo.' }, { status: 500 });
    }
